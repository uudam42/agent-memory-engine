...
print_final_summary() {
    ...
    echo "    5. On first use, Memory Engine will automatically write an agent"
    echo "       workflow policy into your project's CLAUDE.md (or .cursor/rules/)."
    echo "       The policy tells your agent exactly when to call retrieve and reflect."
    echo "       To install it immediately without waiting for first use:"
    echo "       uv run --directory $REPO_ROOT memory policy install \\"
    echo "         --project-root /path/to/your-project --client claude-code"
    echo ""
